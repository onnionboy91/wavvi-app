...
export const addVideo = createAsyncThunk('videos/add', (video: VideoWithOutId) =>
  fetchAddVideo(video),
);

export const removeVideo = createAsyncThunk('categories/remove', (videoId: VideoId) =>
  fetchVideoRemove(videoId),
);
  ...
  extraReducers: (builder: {
    ...
    builder
      .addCase(
        ...
        (state: { videos: any; loading: boolean }, action: { payload: any }) => {
      ...
      .addCase(
        loadVideos.rejected,
        (state: { error: any; loading: boolean }, action: { error: { message: any } }) => {
      ...
      .addCase(
        ...
        (state: { videos: any[] }, action: { payload: string | number }) => {
          state.videos = state.videos.filter(
            (video: { id: number }) => video.id !== +action.payload,
          );
        },
      )
      .addCase(
        removeVideo.rejected,
        (state: { error: any }, action: { error: { message: any } }) => {
          state.error = action.error.message;
        },
      );
  },
